...
98
 */
package dio.gft.desafio3.ordenandoNumerosParesEImpares;
...
public class OrdenandoNumerosParesEImpares {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> valoresLidos = new ArrayList<>();

        int N = leitor.nextInt();
        for (int i = 0; i < N; i++) {

            int valorLido = leitor.nextInt();
            valoresLidos.add(valorLido);
        }
        valoresLidos.stream()
                .filter(valor -> valor % 2 == 0)
                .sorted()
                .forEach(valorPar -> System.out.println(valorPar));

        valoresLidos.stream()
                .filter(valor -> valor % 2 != 0)
                .sorted(Comparator.reverseOrder())
                .forEach(valorImpar -> System.out.println(valorImpar));
    }
}
